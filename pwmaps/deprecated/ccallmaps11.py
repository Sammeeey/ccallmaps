...
def run(playwright: Playwright, targetGroup, searchRegion) -> None:
    browser = playwright.firefox.launch(headless=False, slow_mo=5000) # alll executions slowed down, so that maps doesn't just refuse to load further artciles (DOESN'T WORK: while loop seemingly regarded as one action): https://github.com/microsoft/playwright/issues/5900 & https://playwright.dev/python/docs/api/class-browsertype#browser-type-launch
